...
export const createProject = async (req, res) => {
  ...
  try {
    ...
    const newProject = new Project({
      name: name,
      authorizedUsers: [user._id.toString()],
      tasks: [],
    });
...
export const generateShareableLink = async (req, res) => {
  ...
  // Generate a unique link (could be a token or hash)
  const link = crypto.randomBytes(16).toString("hex");
  project.shareableLink = link;
  await project.save();
  res.json({ shareableLink: link });
};
...
export const addAuthorizedUser = async (req, res) => {
  ...
  const project = await Project.findById(req.params.id);
  if (!project) return res.status(404).json({ error: "Project not found" });
  if (!project.authorizedUsers.includes(userId)) {
    project.authorizedUsers.push(userId);
    await project.save();
  }
  res.json(project);
};

export const removeAuthorizedUser = async (req, res) => {
  const { userId } = req.body;
  const project = await Project.findById(req.params.id);
  if (!project) return res.status(404).json({ error: "Project not found" });
  project.authorizedUsers = project.authorizedUsers.filter(
